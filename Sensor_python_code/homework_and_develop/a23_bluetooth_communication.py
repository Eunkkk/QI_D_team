...
def main_process(num):
    for i in range (0,5):
        ...
        time.sleep(1)
        c.execute("INSERT INTO history VALUES (?,?,?,?,?,?,?)", (datetime.now(), sensor_value[(6*i)+0], sensor_value[(6*i)+1], sensor_value[(6*i)+2], sensor_value[(6*i)+3], sensor_value[(6*i)+4], sensor_value[5]))
        conn.commit()
        conn.close()
        num += 1
        if num == 5:
