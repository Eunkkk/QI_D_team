...
'''
sensor_value = [[0]*5 for i in range(5)]
    ...
    while Running:
        for j in range (0,5):
            ...
            for i in range(0,5):
                ...
                if i == 0:
                    sensor_value[j][0] = cal_temp36(sn_WE)         # C8, TMP36 Sensor
                    sensor_value[j][1] = cal_pm(sn_AE)                # C9, PM2.5 Sensor
                    print "\nTemperature value is {0}".format(sensor_value[j][0])
                    print "PM2.5 value is {0}".format(sensor_value[j][1])
                else:
                    sensor_value[j][i+1] = cal_gas(sn_WE, sn_AE, i)
                    if i == 1: print "NO2 value is {0}".format(sensor_value[j][2])
                    elif i == 2: print "O3 value is {0}".format(sensor_value[j][3])
                    elif i == 3: print "CO value is {0}".format(sensor_value[j][4])
                    elif i == 4: print "SO2 value is {0}\n".format(sensor_value[j][5])    
                
        c.execute("INSERT INTO history VALUES (?,?,?,?,?,?,?)", (datetime.now(), sensor_value[0], sensor_value[1], sensor_value[2], sensor_value[3], sensor_value[4], sensor_value[5]))
        conn.commit()
        count += 1
        print count
