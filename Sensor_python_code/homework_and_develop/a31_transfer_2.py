...
def bluetooth_communication(send_data, send_aqi):
    ...
    '''
    for k in range (0,11):
        client_sock.send(str(k) + '-st ' + str(temp_data[k]) + '\n')
        time.sleep(0.010)
    '''
    '''
...
def main_process(num):
    for i in range (0,5):
        ...
        num += 1
        if num == 5:
            final_aqi(result_aqi)
            bluetooth_communication(sensor_value, result_aqi,final_aqi)
            num = 0

def final_aqi(need_cal_aqi):
