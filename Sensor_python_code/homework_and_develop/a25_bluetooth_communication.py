...
def read_avg_sqi_DB():
    ...
    print tmp[0]
    print '\n'
    result_aqi[0] = o3_to_aqi(-1*tmp[0])
    
    print result_aqi[0]
    print '\n'
    
    c.execute("SELECT AVG(o3) FROM history ORDER BY time DESC LIMIT 5")
    avg_o3_8hr = c.fetchone() 
    ...
    tmp[5] = avg_no2_1hr[0]
    result_aqi[5] = no2_to_aqi(tmp[5])
    print "avg_value is : {0}       aqi_value is : {1}".format(tmp[5],result_aqi[5])

    
    c.execute("SELECT AVG(no2) FROM history ORDER BY time DESC LIMIT 5")
    avg_no2_1hr = c.fetchone() 
    tmp[6] = avg_no2_1hr[0]
    result_aqi[6] = no2_to_aqi(tmp[6])
    print "avg_value is : {0}       aqi_value is : {1}".format(tmp[6],result_aqi[6])
